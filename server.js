    ...
    const defaultTemplates = [
      {
        ...
        instructions: "Review the provided code for quality, performance, security, and maintainability",
        ...
        expectations: "Detailed review with specific line-by-line feedback and actionable suggestions",
        narrowing: "Focus on critical issues first, then style and minor improvements",
        variables: JSON.stringify(["code_snippet", "programming_language", "context"]),
      ...
      },
      {
        ...
        instructions: "Write an engaging blog post about {{topic}} targeting {{audience}}",
        steps: JSON.stringify([
        ...
        expectations: "1500-2000 word blog post, SEO-optimized, engaging tone, well-researched",
        narrowing: "Use conversational tone, include 3-5 keywords naturally, target readability score of 60+",
        variables: JSON.stringify(["topic", "audience", "industry", "keywords"]),
      ...
      },
      {
        ...
        instructions: "Analyze the {{dataset_description}} to uncover insights and patterns",
        steps: JSON.stringify([
        ...
        expectations: "Clear insights with statistical backing, visualization suggestions, business recommendations",
        narrowing: "Focus on {{specific_metrics}} and their business impact",
        variables: JSON.stringify(["domain", "dataset_description", "specific_metrics"]),
        tags: JSON.stringify(["data", "analysis", "insights", "statistics"])
      }
    ];

    console.error(`[RISEN] Creating ${defaultTemplates.length} default templates...`);
    let created = 0;
    let skipped = 0;

    for (const template of defaultTemplates) {
      try {
        const existing = await dbGet('SELECT id FROM templates WHERE name = ?', template.name);
        if (!existing) {
          await dbRun(
        ...
        console.error(`[RISEN] Failed to create template "${template.name}":`, templateError.message);
        // Continue with other templates instead of failing completely
      }
    }
    
    console.error(`[RISEN] Default templates processed: ${created} created, ${skipped} skipped`);
...
function validateRISEN(template) {
  ...
  if (!template.role || template.role.trim().length < 10) {
    errors.push("Role must be at least 10 characters and clearly define the AI's persona");
  }
  
  if (!template.instructions || template.instructions.trim().length < 20) {
    errors.push("Instructions must be at least 20 characters and provide clear directives");
  }
  ...
  if (!template.expectations || template.expectations.trim().length < 15) {
    errors.push("Expectations must clearly define the desired outcome (min 15 chars)");
  }
  
  if (!template.narrowing || template.narrowing.trim().length < 10) {
    warnings.push("Narrowing/Novelty helps focus or expand the task - consider adding constraints or creative elements");
  }
  ...
  [template.role, template.instructions, template.expectations, template.narrowing].forEach(text => {
    if (text) {
...
function calculateQualityScore(template) {
  ...
  if (template.instructions && template.instructions.includes('{{')) score += 10; // Uses variables
  ...
  if (template.narrowing && (template.narrowing.includes('avoid') || template.narrowing.includes('focus'))) score += 10;
...
function generateSuggestions(template) {
  ...
  if (template.role && template.role.length < 30) {
    suggestions.push({
      component: "role",
      suggestion: "Consider adding more specific expertise, years of experience, or domain knowledge to the role"
    });
  }
  ...
  if (template.instructions && !template.instructions.includes('{{')) {
    suggestions.push({
      ...
      suggestion: "Consider using variables (e.g., {{topic}}) to make this template reusable"
    });
  }
  ...
  try {
    ...
    if (steps.length < 3) {
      suggestions.push({
        component: "steps",
        suggestion: "Break down the task into more detailed steps for better AI guidance"
      });
    }
  ...
  if (template.expectations && !(/\d+/.test(template.expectations))) {
    suggestions.push({
      ...
      suggestion: "Consider adding specific metrics or measurable outcomes (e.g., word count, number of examples)"
    });
  }
  ...
  if (!template.narrowing || template.narrowing.length < 20) {
    suggestions.push({
      component: "narrowing",
      suggestion: "Add constraints to focus the output or creative elements to encourage innovation"
    });
  }
  ...
  tools: [
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          steps: { type: 'array', items: { type: 'string' }, description: 'Task breakdown' },
          expectations: { type: 'string', description: 'Desired outcome' },
          narrowing: { type: 'string', description: 'Constraints or creative elements' },
    ...
    {
      name: 'risen_validate',
      description: 'Validate a RISEN prompt structure and get improvement suggestions',
      inputSchema: {
        type: 'object',
        properties: {
          template_id: { type: 'string', description: 'Template ID to validate' },
          template: {
            type: 'object',
    ...
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          variables: { type: 'object', description: 'Variables to fill in template' }
        },
    ...
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          template_id: { type: 'string', description: 'Template ID' },
          executed_prompt: { type: 'string', description: 'The executed prompt' },
          variables_used: { type: 'object', description: 'Variables that were used' },
          ai_model: { type: 'string', description: 'Which AI model was used' },
          response: { type: 'string', description: 'AI response (truncated if needed)' },
    ...
    },
    {
      ...
      inputSchema: {
        ...
        properties: {
          ...
          offset: { type: 'integer', minimum: 0, default: 0, description: 'Pagination offset (starting record)' },
          limit: { type: 'integer', minimum: 1, maximum: 100, default: 20, description: 'Number of results per page (max 100)' }
        }
      }
    },
    {
      name: 'risen_analyze',
      description: 'Analyze template performance and get insights with pagination',
      inputSchema: {
        type: 'object',
        properties: {
          template_id: { type: 'string', description: 'Template ID to analyze' },
          offset: { type: 'integer', minimum: 0, default: 0, description: 'Pagination offset for experiments' },
          limit: { type: 'integer', minimum: 1, maximum: 50, default: 10, description: 'Number of experiments per page (max 50)' }
        },
        required: ['template_id']
      }
    },
...
${validation.warnings.length > 0 ? '\n⚠️ Suggestions:\n' + validation.warnings.join('\n') : ''}`
      ...
      case 'risen_validate': {
        let template;
        if (args.template_id) {
          template = await dbGet('SELECT * FROM templates WHERE id = ?', args.template_id);
          if (!template) {
          ...
          return { content: [{ type: 'text', text: '❌ Provide either template_id or template object' }] };
        }
...
${suggestions.map(s => `• [${s.component.toUpperCase()}] ${s.suggestion}`).join('\n')}`
      ...
      case 'risen_execute': {
        const template = await dbGet('SELECT * FROM templates WHERE id = ?', args.template_id);
        if (!template) {
          return { content: [{ type: 'text', text: '❌ Template not found' }] };
        }
        ...
        // Update usage count
        await dbRun('UPDATE templates SET uses = uses + 1 WHERE id = ?', args.template_id);
      ...
      case 'risen_track': {
        ...
        const updatedTemplate = await dbGet('SELECT * FROM templates WHERE id = ?', args.template_id);
        const avgRating = updatedTemplate.total_rating / updatedTemplate.rating_count;
...
${args.notes ? `Notes: ${args.notes}` : ''}`
      ...
      case 'risen_search': {
        ...
        if (templates.length === 0 && totalCount === 0) {
          return { content: [{ type: 'text', text: '❌ No templates found matching your criteria' }] };
        }
        
        // Format results with pagination metadata
        const results = templates.map(t => {
          const avgRating = t.rating_count > 0 ? (t.total_rating / t.rating_count).toFixed(1) : 'N/A';
...
📄 Page ${currentPage} of ${totalPages} | Showing ${offset + 1}-${offset + templates.length} of ${totalCount} results${hasMore ? `\n\n➡️ Use offset: ${offset + limit} for next page` : ''}`
      ...
      case 'risen_analyze': {
        ...
        const totalExperiments = experimentCountResult ? experimentCountResult.total : 0;
        ...
        const modelAnalysis = Object.entries(modelStats).map(([model, stats]) => {
...
${notes.length > 0 ? notes.slice(0, 3).map(n => `   • ${n}`).join('\n') : '   No feedback notes yet'}
...
${avgRating < 3 ? '   • Consider refining the prompt structure\n' : ''}${template.uses < 5 ? '   • Need more usage data for reliable insights\n' : ''}${notes.length < 3 ? '   • Encourage users to leave feedback notes\n' : ''}${hasMoreExperiments ? `\n➡️ Use offset: ${offset + limit} to see more experiments` : ''}`
      ...
      case 'risen_suggest': {
        ...
        if (avgRating && parseFloat(avgRating) < 3) {
          enhancedSuggestions.push("Low ratings suggest the prompt may be too vague or complex. Consider simplifying.");
        }
        
        if (template.uses > 10 && (!avgRating || parseFloat(avgRating) < 4)) {
          enhancedSuggestions.push("With many uses but moderate ratings, gather user feedback to identify specific issues.");
        }
        
        // Component-specific enhanced suggestions
        const roleWords = template.role.split(' ').length;
        if (roleWords < 8) {
          enhancedSuggestions.push("Role is very brief. Try: 'Expert [domain] professional with [X] years experience in [specific areas], specialized in [unique skills]'");
        }
        
        try {
          const steps = JSON.parse(template.steps);
          if (steps.some(s => s.split(' ').length < 5)) {
            enhancedSuggestions.push("Some steps are too brief. Each step should be a complete, actionable instruction.");
          }
        } catch (e) {}
...
• Test with different variable combinations to find what works best`
      ...
      case 'risen_convert': {
        ...
        const keywords = {
          ...
          expectations: ['should be', 'must include', 'make sure', 'i want', 'need'],
        ...
        let narrowing = "Focus on practical solutions";
        ...
        for (const keyword of keywords.role) {
          if (request.toLowerCase().includes(keyword)) {
            ...
            if (endIdx > startIdx) {
              role = request.substring(startIdx, endIdx).replace(keyword, '').trim();
              break;
            }
          }
        }
...
async function main() {
  ...
  while (retries < maxRetries) {
    try {
      ...
      console.error(`[RISEN] Database initialization failed (attempt ${retries + 1}/${maxRetries}):`, error.message);
      retries++;
  ...
  if (retries === maxRetries) {
    console.error('[RISEN] Database initialization failed after multiple retries. Exiting.');
